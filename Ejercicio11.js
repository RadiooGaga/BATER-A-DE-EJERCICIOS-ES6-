...
const favoriteSoundsAdded = {};

for (const user of users){
    for (const sound in user.favoriteSounds){
        if (!typeOfSounds.includes(sound)) {
            typeOfSounds.push(sound);
            soundsCounter.push(1);
          } else {
            const soundPosition = typeOfSounds.indexOf(sound);
            soundsCounter[soundPosition]++;
          }
    }  
}
for (let i = 0; i < typeOfSounds.length; i++) {
  favoriteSoundsAdded[typeOfSounds[i]] = soundsCounter[i];
}

console.log('Sonidos favoritos de los usuarios y sus repeticiones', favoriteSoundsAdded);
